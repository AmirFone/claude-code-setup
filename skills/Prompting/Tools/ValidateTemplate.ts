...
function resolveTemplatePath(path: string): string {
  ...
  // Get the Prompting skill root directory (parent of Tools/)
  const skillRoot = dirname(dirname(import.meta.path));
  // For paths starting with Primitives/ or Templates/, resolve from skill root
  if (path.startsWith('Primitives/') || path.startsWith('Data/')) {
    return resolve(skillRoot, 'Templates', path);
  }
  // For paths starting with ../, resolve from CWD
  if (path.startsWith('../') || path.startsWith('./')) {
    return resolve(process.cwd(), path);
  }
  // Default: resolve from skill root
  return resolve(skillRoot, path);
}
